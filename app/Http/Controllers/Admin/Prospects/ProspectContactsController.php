...
class ProspectContactsController extends Controller
{
    ...
    public function store(StoreContactRequest $request, Prospect $prospect)
    {
        ...
        return redirect()->route('admin.prospects.show', $prospect->id)->with('success', 'Успешно создана новая перспектива!');
    }
    ...
    public function update(UpdateContactRequest $request, Prospect $prospect)
    {
        ...
        return redirect()->route('admin.prospects.edit', $prospect->id)->with('success', 'Контактные данные потенциального клиента обновлены!');
    }
}
